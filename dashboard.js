...
const refreshToken = async () => {
  ...
  try {
    ...
    var response = await fetch(`https://freddy.codesubmit.io/refresh`, {
      method: "POST",
    ...
    if (data.access_token) {
      ...
      var errhandler = (document.getElementsByClassName(
        "login-error"
      )[0].innerHTML = data.msg);
      var errColor = (document.getElementsByClassName(
        "login-error"
      ).style.color = "red");
      errColor;
      errhandler;
...
switchButton.addEventListener("click", function () {
  if (switchState === "month") {
    ...
    chartTitle.innerHTML = 'Revenue (Last 7 Days)'

    var chart = document.getElementsByClassName("weekly-sales-row")[0];
    ...
    for (const key in weeklySales) {
      ...
      document.getElementsByClassName("weekly-sales-row")[0].appendChild(td);
    }
    ...
    chartTitle.innerHTML = "Revenue (Last 12 Months)"
...
const dashboard = async () => {
  ...
  try {
    ...
    bestSellers.forEach((element) => {
      ...
      td1.innerHTML =
        element.product.name.length > 15
          ? element.product.name.slice(0, 15) + "..."
          : element.product.name;
      td2.innerHTML = "";
      td3.innerHTML = element.units;
      td4.innerHTML = element.revenue;

      document.getElementsByClassName("bs-table")[0].appendChild(tr);
    });
